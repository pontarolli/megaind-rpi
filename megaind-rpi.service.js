...
broker.createService({
    ...
    actions: {   
        setOutputVoltage: {    
            ...
            async handler(ctx) {return await this.setMemoryWord(I2C_MEM_U0_10_OUT_VAL1, ctx.params.stack, ctx.params.channel, this.percentageToVoltage(ctx.params.value))}
        },
        getOutputVoltage: {            
			params: { 
				stack  : { type: "number", integer: true, min: 0, max: 7},
        ...
        setOutputCurrent: {    
            ...
            async handler(ctx) {return await this.setMemoryWord(I2C_MEM_I4_20_OUT_VAL1, ctx.params.stack, ctx.params.channel, this.percentageToCurrent(ctx.params.value/10))}
        },
        getOutputCurrent: {    
            params: {
                stack  : { type: "number", integer: true, min: 0, max: 7},
        ...
        setOutputOpenDrain: {
            ...
            async handler(ctx) {return await this.setMemoryWord(I2C_MEM_OD_PWM1, ctx.params.stack, ctx.params.channel, ctx.params.value/10)}
        },
        getOutputOpenDrain: {
            params: {
                stack  : { type: "number", integer: true, min: 0, max: 7},
        ...
        getInputOpto : {
            ...
            async handler(ctx) {return await this.getMemoryByte(I2C_MEM_OPTO_IN_VAL, ctx.params.stack, ctx.params.channel)}   
        },     
    ...
    methods: {
        ...
        percentageToCurrent(percentage){
            return +((percentage*1.6)).toFixed(2)
        },
